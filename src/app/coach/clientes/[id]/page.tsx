...
export default function ClientSummaryPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const state = useDemo();
  const person = state.clients.find((item) => item.id === id);

  useEffect(() => {
    if (person && state.selectedClientId !== person.id) {
      state.dispatch({ type: "SELECT_CLIENT", id: person.id });
    }
  }, [person, state.selectedClientId, state.dispatch]);

  if (!person) {
    return <Empty title="Cliente no encontrado" body="Volvé al listado e elegí otro cliente." />;
  }
  ...
  const lastJournals = [...scoped.journals].slice(-7);
  const avgDiet =
  ...
  const lastWeight = [...scoped.measurementEntries]
    .reverse()
    .map((entry) =>
      Object.entries(entry.values).find(([key, value]) => key.includes("mf-peso") && typeof value === "number")?.[1],
    )
    .find((value) => typeof value === "number");
  const volume = meso ? weekVolume(state, meso.id, Math.max(1, week - 1)) : 0;
  const lastJournal = scoped.journals.at(-1);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        ...
        <h1 className="font-display text-4xl italic">{person.name}</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <ul className="space-y-3">
            {scoped.sessions.slice(-4).reverse().map((session) => {
              const day = scoped.trainingDays.find((item) => item.id === session.trainingDayId);
              const kg = scoped.setLogs
